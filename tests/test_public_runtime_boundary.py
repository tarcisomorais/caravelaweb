...
four public CLI entry points and asserts the reachable set is exactly the
eleven runtime modules this repository ships -- no more, no fewer.

It also asserts that no unreachable Python module sits at the runtime level,
so a file that nothing imports cannot ship unnoticed alongside the runtime.
"""
...
class PublicRuntimeBoundaryTests(unittest.TestCase):
    def test_public_runtime_import_closure_is_exact(self) -> None:
        ...
        entered = reached - EXPECTED_CLOSURE
        left = EXPECTED_CLOSURE - reached
        self.assertEqual(
            EXPECTED_CLOSURE,
            reached,
        ...
        unreachable = set(modules) - reached
        self.assertEqual(
            set(),
            unreachable,
